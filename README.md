...
罗盘解锁小地图和目标箭头，受伤时可能掉落。油灯照明 20 秒；玉衣只能抵挡一次伤害。榆木护盾拥有 3 点耐久，会自动格挡弓弩箭矢和守墓尸攻击，但无法抵挡火焰、巨石或滚木。每层物品有限，大部分供物与僵尸藏在棺材中；普通棺有概率掉落写实古董冥器，主棺保证掉落一件，并同时释放高血量赤血厉尸与尸鳖群。僵尸或爬尸破棺后，原棺会切换为独立的写实空棺素材。玩家按住手捏鼻子的屏气按钮后，僵尸会转身面向原棺返回；完全沉入后棺盖自动闭合并锁定。关卡加载时会按墓室中轴和墙边陈设规则调整棺材、罐子、石碑、遗骸与机关间距，避免物体彼此重叠或遮挡。冥器以万元估值，过关界面会汇总全部藏品与总价值。封闭陪葬密室没有灯光，开启前场景和小地图均不可见；入口墙没有提示或特殊边框，人物必须紧贴墙面，石墙才会用约四秒缓慢打开且不再关闭。每间密室保证放置一件陪葬冥器和一具补给棺，第一间密室可找到榆木护盾。拾取物品或冥器时会弹出图像、用途、耐久或估值等详细信息。兵工铲会自动攻击近处僵尸与大罐，挥击有冷却且不能穿墙；屏气时自动攻击暂停。爬尸使用独立的四帧爬行动画。
