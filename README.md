...
首关进入后显示拖动移动示范，实际移动后自动隐藏。十层固定地图及棺材位置保存于 `levels.js`，现为 72×72 到 144×144 格：保留各层原有拓扑和汉墓对称结构，同时将房间边长扩大约 50%、房间中心间距扩大一倍，使墓道明显加长；重新运行不会重排地图或棺材物品。

## 探索与战斗

罗盘解锁小地图和目标箭头，受伤时可能掉落。油灯照明 20 秒；玉衣只能抵挡一次伤害。每层物品有限，大部分供物与僵尸藏在棺材中，主棺开启后出现的是本层普通守墓僵尸。封闭密室没有灯光，开启前场景和小地图均不可见；入口墙没有提示或特殊边框，人物必须紧贴墙面，石墙才会用约四秒缓慢打开且不再关闭。兵工铲会自动攻击近处僵尸与大罐，挥击有冷却且不能穿墙；屏气时自动攻击暂停。
...
Node.js 22 以上运行 `node --test tests/*.test.cjs`。73 项测试覆盖扩展地图、物品、陷阱、渐亮墓室、隐藏墙、大罐、血尸与尸鳖群、自动攻击、屏气与控制逻辑。测试使用浏览器接口替身，不能代替手机实机触控与音频验收。
