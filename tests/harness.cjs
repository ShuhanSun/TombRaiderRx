...
const source=['audio.js','escape.js','expedition.js','world.js','art.js','transitions.js','game.js'].map(f=>fs.readFileSync(path.join(root,f),'utf8')).join('\n');
...
function setup(seed=1) {
    ...
    vm.runInContext(source+'\nthis.api={Game,MapSys,Input,Player,Coffin,Zombie,Trap,Projectile,Effect,FloatText,AudioSys,CONFIG,TERRAIN,World,Art,Scene,Sound,THEMES,Passage,ExitGate,SPECIES,FLOOD_TYPES,Expedition,TombCreature,RELIC_VALUES};',context);
